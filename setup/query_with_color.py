...
parser.add_argument(
    "--model-name", type=str, default="yolov5l", help="YOLOv5 model name"
)
parser.add_argument(
...
def isObject(start, end, chunk_size, step_size, object, color, argum):
    # detecting the model
            ...
            if prev_hist is None or cv2.compareHist(hist, prev_hist, cv2.HISTCMP_INTERSECT) < HISTOGRAM_THRESHOLD:
                ...
                for obj in tracked_objects:
                    if obj.label == object:
                        ...
                        dominant_color = extract_color_from_bbox(frame, x1,y1,x2,y2)
                        closest_color_of_obj = closest_color(dominant_color)
                        if closest_color_of_obj == color:
            ...
            if frame_count == chunk_size * fps:
                if valid_frames > 0:
                    ...
                    for olist in output:
                        for obj in olist:
                            do.add(obj)
                    ans.append((start_frame / fps - chunk_size, start_frame / fps, valid_frames, len(objects)))
                    do.clear()
...
object_id = 2  # Object ID for car
color_to_detect = "YELLOW"  # Color to detect
result = isObject(0, 30, 5, 1, object_id, color_to_detect, args)
